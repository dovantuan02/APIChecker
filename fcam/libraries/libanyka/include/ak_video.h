...
typedef enum VI_VENC_ENC_MODE_E
{
    USER_ENCODE_MODE = 0x0,
    KERNEL_ENCODE_MODE,
}VI_VENC_ENC_MODE_T;
...
typedef struct ak_app_chn_s
{
    ...
    /* object id, chn id for vi, encoder id for venc,
    decoder id for vdec, layer id for vo */
    int             oid;
}APP_CHN_S;
...
typedef struct ak_app_bind_param
{
    ...
    VI_VENC_ENC_MODE_T      mode;                   /* bind mode */
}APP_BIND_PARAM;
 ...
 * return AK_SUCCESS is successful,  error code otherwise.
 */
int ak_app_video_venc_get_stream(const APP_CHN_S *Chn,
 ...
 * return AK_SUCCESS is successful,  error code otherwise.
 */
int ak_app_video_unbind_chn(const APP_CHN_S *pstSrcChn,
                                const APP_CHN_S *pstDestChn);
 ...
 * return AK_SUCCESS is successful,  error code otherwise.
 */
int ak_app_video_bind_chn(const APP_CHN_S *pstSrcChn,
                const APP_CHN_S *pstDestChn, const APP_BIND_PARAM *param);
 ...
 * return AK_SUCCESS is successful,  error code otherwise.
 * NOTE : This api is only to modify the frame rate output from vi.
 *        for the venc wanted, call the  corresponding venc interface
 */
int ak_app_video_set_chn_attr(const APP_CHN_S *pstDestChn,
                                const APP_BIND_PARAM *param);
