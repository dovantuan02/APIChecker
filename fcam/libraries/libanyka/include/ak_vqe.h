...
enum ak_vqe_error_type
{
    ...
    ERROR_VQE_NO_MATCH_VAD_TYPE,
    ERROR_VQE_RESULT_NO_RETURN
};
...
/**
 * ak_vqe_get_filter_info - get audio filter lib version
 * return: audio filter lib version
 * notes:
 */
char *ak_vqe_get_filter_info(void);

/**
 * ak_vqe_set_pitch_attr - voice quality enhancement module set pitch attribute
 * @vqe_handle_id[IN]: vqe module opened handle id
 * @pcm_attr_in[IN]: audio data attribute
 * @pitch_attr[IN]: pitch attribute
 * return: 0 success, other failed
 * notes:
 */
int ak_vqe_set_pitch_attr(int vqe_handle_id,
                           struct ak_audio_data_attr *pcm_attr_in,
                           struct ak_audio_pitch_attr *pitch_attr);

/**
 * ak_vqe_set_tw_attr - tone detection module set  attribute
 * @vqe_handle_id[IN]: vqe module opened handle id
 * @pcm_attr_in[IN]: aslc input, audio data attribute
 * return: 0 success, other failed
 * notes:
 */
int ak_vqe_set_tw_attr(int vqe_handle_id,
                         struct ak_audio_data_attr *pcm_attr_in);
