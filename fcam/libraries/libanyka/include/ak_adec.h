...
struct adec_aac_dec_param
{
    unsigned short aac_stream_format;// 0: auto detect; 1: adts; 2: adif; 3: raw
};
 ...
 * return: 0 success; other failed
 */
int ak_adec_get_frame(int adec_handle_id, struct frame *pcm_frame, \
    unsigned char block);
...
/**
 * ak_adec_save_file - save 4 files, one file is stream input to adec,
 *two file is pcm output from adec, three is stream before decode,
 *four is pcm after decode
 * @adec_handle_id[IN]: adec handle id number
 * @save_file_info[IN]: save file infomation
 * return: 0 success, other failed
 * notes:
 */
int ak_adec_save_file(int adec_handle_id, \
 ...
 * @buf_status[OUT]: get decode buffer total length and
 * decode buffer remain length
 * return: 0 success, otherwise failed
 * notes:
 */
int ak_adec_get_buf_status(int adec_handle_id, \
    struct ak_dec_buf_status *buf_status);

/**
 * ak_adec_set_aac_dec_param - audio decode set aac decode parameter
 * @ao_handle_id[IN]: adec opened handle id
 * @dec_param[IN]: get decode buffer total length and \
 * decode buffer remain length
 * return: 0 success, otherwise failed
 * notes: this function will reopen audio decode lib, \
 * must use before ak_adec_send_stream
 */
int ak_adec_set_aac_dec_param(int adec_handle_id, \
    struct adec_aac_dec_param *dec_param);
