...
#ifdef AK_RTOS
static const char pcm_def_parth[] = "/mnt/sdcard/ak_ao_test.pcm";
#else
...
/* when sample rate more than 44100, the frame length which send to ao must more than 8192 byte */
        data_len = PCM_READ_LEN << 1;
    }

    if (AK_AUDIO_SAMPLE_RATE_48000 < sample_rate) {
    /* when sample rate more than 48000, the frame length which send to ao must more than 16384 byte */
        data_len = PCM_READ_LEN << 2;
    }
