...
* File Name: ak_vqe_sample.c
* Description: This is a simple example to show how the VQE module working.
...
static int vqe_pitch_handle_id = -1; // pitch handle
static int pitchTempo = 7; // pitchTempo
/* ***********************************
    ***********************************
    *
    use this sample
...
static struct option option_long[] = {
    ...
    { "pitchTempo", required_argument , NULL , 'i' } , //"[PITCH_TEMPO] 变声参数选择" ,
    {0, 0, 0, 0}
};
...
static int help_hint(char *pc_prog_name)
{
    ...
    printf( "\nexample: \r\n"
    "  %s -t 60 -s 8000 -c 1 -v 5 -d 0 -p /mnt/ - i 7\n", pc_prog_name);
    printf("\n\n");
    return 0;
}
 ...
 */
static int check_dir(const char *path)      // not used
{
    struct stat buf = {0};
 ...
 * return: void.
 */
static void create_pcm_file_name(const char *path, char *file_path,     //not used
...
static void setup_default_audio_argument(void *audio_args, char args_type)
{
    ...
    {TYPE_HPF, TYPE_PF1, TYPE_PF1, TYPE_PF1, TYPE_PF1, TYPE_PF1,\
    TYPE_PF1, TYPE_PF1, TYPE_PF1, TYPE_PF1},
    0,
    0,
    0,
    0,
...
/**
 * init_vqe_pitch - open vqe, init pitch parameter
 * notes:
 */
static int init_vqe_pitch(void)
{
#if defined(__CHIP_AK3918AV100_SERIES) && !defined(AK_RTOS)
    /* open vqe */
    if (ak_vqe_open(AK_VQE_PITCH, &vqe_pitch_handle_id)) {
        return AK_FAILED;
    }
    struct ak_audio_data_attr pcm_attr_in;
    ...
    struct ak_audio_pitch_attr vqe_pitch_attr = {0};
    vqe_pitch_attr.pitchMode = VQE_PITCH_CHILD_VOICE;
    /* pitchTempo 参数的范围是[0-10], 0~4提高音调，5正常音调，6~10降低音调 */
    /* 童声是2，男声是6或7 */
    vqe_pitch_attr.pitchTempo = pitchTempo;

        // call ak_pitch_register before ak_vqe_set_pitch_attr
        ak_pitch_register();

    if (ak_vqe_set_pitch_attr(vqe_pitch_handle_id, &pcm_attr_in, &vqe_pitch_attr)) {
        ak_print_error_ex(MODULE_ID_APP, "\nERR: ak_vqe_set_pitch_attr failed!\n\n");
        return AK_FAILED;
    }
    return AK_SUCCESS;
#else
    ak_print_error_ex(MODULE_ID_APP, "\nERROR: not support pitch\n\n");
    return AK_FAILED;
#endif
}

/**
 * destroy_vqe_pitch - close vqe
 * notes:
 */
static void destroy_vqe_pitch(void)
{
    if (-1 != vqe_pitch_handle_id) {
        /* vqe close */
        ak_vqe_close(vqe_pitch_handle_id);
        vqe_pitch_handle_id = -1;
    }
}
/*
...
static void* ai_capture_loop(void *arg)
{
    ...
    unsigned char *pitch_data = NULL;
    unsigned int pitch_len = 0;
    ...
    while (1) {
...
#ifdef __CHIP_AK3918AV100_SERIES
    #ifdef AK_RTOS
        ret = ak_vqe_process(vqe_eq_handle_id, frame.data, frame.len, &eq_data, &eq_len);
        if (ret)
            break;
    #else
        ret = ak_vqe_process(vqe_pitch_handle_id, frame.data, frame.len, &pitch_data, &pitch_len);
        if (ret){
            break;
        } else {
            if(0 == pitch_len){
        ...
        ret = ak_vqe_process(vqe_eq_handle_id, pitch_data, pitch_len, &eq_data, &eq_len);
        if (ret)
            break;
    #endif
#else
...
#endif
{
    ...
    if (init_vqe_pitch()) {
        ak_print_error_ex(MODULE_ID_APP, "init vqe pitch failed\n");
        goto vqe_end;
    }
    if (init_vqe_eq()) {
