...
* File Name: ak_vad_sample.c
* Description: This is a simple example to show how the VAD module SOUND_LEVEL working.
...
/* probe_mode=1 is vad probe mode, probe_mode=0 is vad normal mode */
static int probe_mode = 0;
/* vad thresh in FS, vad thresh in FS is vad_thresh/AK32Q15(1), 10 times vad thresh in FS add together */
static float vad_thresh_fs_sum = 0;
...
static void* ai_capture_loop(void *arg)
{
    ...
    if (probe_mode)
        vad_status.vadType = AK_VAD_TYPE_PARAM(AK_VAD_DET_AFTER_ADC, AK_VAD_DET_SOUND_LEVEL , 0, AK_VAD_DET_PROBE);
    else
        vad_status.vadType = AK_VAD_TYPE_PARAM(AK_VAD_DET_AFTER_ADC, AK_VAD_DET_SOUND_LEVEL , 0, 0);
...
static int set_vad_param(void)
{
    ...
    if (probe_mode)
        vad_attr.vad_type = AK_VAD_TYPE_PARAM(AK_VAD_DET_AFTER_ADC, AK_VAD_DET_SOUND_LEVEL , 0, AK_VAD_DET_PROBE);
    else
        vad_attr.vad_type = AK_VAD_TYPE_PARAM(AK_VAD_DET_AFTER_ADC, AK_VAD_DET_SOUND_LEVEL , 0, 0);
