 ...
 * File Name: ak_venc_sample.c
 * Description: Sample for encoding video from YUV file or realtime vi channel yuv stream to H264/H265/JPEG stream.
 * Notes: Before running this example please insmod sensor_xxx.ko ak_isp.ko ak_venc_adapter.ko at first.
 * History: V1.0.0
 */
#include <stdio.h>
#include <string.h>
...
static char ac_option_hint[  ][ LEN_HINT ] = {                                         //操作提示数组
    ...
    "isp 配置文件或YUV源文件目录, (dev_cnt:1  isp_xxx.conf, dev_cnt:2  isp_xxx_dual.conf)",
    "[NUM] device count [1 - 2], default 1" ,
    "[NUM] cycle time, default 1" ,
...
static struct option option_long[ ] = {
    ...
    { "save_file_flag"  , required_argument , NULL , 'f' } , //"[NUM]		save file flag, 0 not save, otherwise save, default 1",
    { 0                   , 0                 , 0    , 0   } ,
};
...
static void usage(const char * name)
{
    ...
    printf("resolution -m -s:     value 0 ~ 10\n");
    printf("                      0 - 640*360\n");
    printf("                      1 - 640*480\n");
    printf("                      2 - 1280*720\n");
    printf("                      3 - 960*1080\n");
...
run_again:
    ...
    if (save_file_flag)
    {
        ...
        memset(venc_th->save_file, 0x00, sizeof(venc_th->save_file));
        if (H264_ENC_TYPE == venc_th->en_type)
            sprintf(venc_th->save_file, "%s/venc_br%d_%dkbps_chn%d_%ux%u.h264", save_path, br_mode,target_kbps,chn_index,res.width, res.height);
        else
            sprintf(venc_th->save_file, "%s/venc_br%d_%dkbps_chn%d_%ux%u.h265", save_path, br_mode,target_kbps,chn_index, res.width, res.height);
    ...
    ak_print_notice_ex(MODULE_ID_APP, "Test run [%d] Cycle!\n",++run_cycle);
    if( run_cycle < cycle )
        goto run_again;
...
static int start_venc(int dev_id)
{
    ...
    ve_param.max_kbps    = target_kbps;        //max kbps
    ve_param.br_mode     = br_mode; //br mode
    ve_param.minqp       = 20;          //qp set
    ve_param.maxqp       = 48;          //qp max value
    ve_param.initqp       = 50;    //qp value
    ...
    ak_venc_strictlylimit_frameSize(handle_id[dev_id],400);
...
#endif
{
    ...
    {
        ak_print_error_ex(MODULE_ID_VENC, "please input the correct vi_fps (1 ~ 30)\n");
        /* print the hint and notice */
        help_hint();
        return AK_FAILED;
    }
