...
static char ac_option_hint[  ][ LEN_HINT ] = {
    ...
    "[NUM]      人形人脸检测模型，0：320*180，1：480*270，2：640*360，3：640*360低性能模式",
...
static int start_svp(void)
{
    ...
    /* init the svp attr */
    AK_SVP_CHN_ATTR_T   attr={0};
    attr.target_type = AK_SVP_HUMAN_SHAPE_AND_FACE;

    if(svp_mode == 0)
        attr.model_type = AK_SVP_MODEL_NORMAL;
    else if(svp_mode == 1)
        attr.model_type = AK_SVP_MODEL_HIGH_PERF;
    else if(svp_mode == 2)
        attr.model_type = AK_SVP_MODEL_RES_360P;
    else if(svp_mode == 3)
        attr.model_type = AK_SVP_MODEL_RES_360P_LOW_PERF;

    /* set svp classsify threshold, default is 700, range[400-900], the less, the more sensitive, but the less accurate */
    attr.threshold.classify_threshold = 700;
    /* set the IoU threshold, range [3-10], default is 3*/
    attr.threshold.IoU_threshold = 3;
    ...
    ret = ak_svp_create_chn(svp_chn_id, &attr, NULL, svp_path);
    if(ret != AK_SUCCESS)
    {
...
static void svp_loop(int number, VI_CHN_ATTR_EX *chn_attr)
{
    ...
    input.img_type = (chn_attr->data_type == VI_DATA_TYPE_YUV420SP ? AK_SVP_IMG_YUV420SP : AK_SVP_IMG_RGB_LI);
    ...
    while (count <  number)
    {
        ...
        if (!ret)
        {
            if(svp_rate != 0 && ++vi_cnt % svp_rate == 0)
            {
                ...
                ret = ak_svp_process(svp_chn_id, &input, &output);
                if(ret == AK_SUCCESS)
                {
                    if(output.total_num > 0)
                    {
                        ...
                        for(;i<output.total_num; i++)
                        {
                            ...
                            i, src->label ==  AK_SVP_HUMAN_SHAPE ?"Body":"Face",
                                ...
                                if(ret_info.result > 0)
                                {
                                    ...
                                    for(j = 0; j< output.total_num; j++)
                                    {
                                        ...
                                        for(i = 0; i< ret_info.move_box_num; i++)
                                        {
                                            ...
                                            md.x2 = (ret_info.boxes[i].right == ret_info.boxes[i].left ?ret_info.boxes[i].right  \
                                            + 1:ret_info.boxes[i].right) * chn_attr->res.width/32 ;
                                            md.y2 = (ret_info.boxes[i].bottom == ret_info.boxes[i].top ? ret_info.boxes[i].bottom \
                                            + 1:ret_info.boxes[i].bottom) * chn_attr->res.height/24;
...
"SVP Alarm[%d] :Mobile [%s] detect, score[%lu],\
Res is:left[%lu],top[%lu],right[%lu]:bottom[%lu]\n",
                                                j, src->label ==  AK_SVP_HUMAN_SHAPE ?"Body":
                                                "Face",src->score, src->left, src->top, src->right, src->bottom);
                                                if(filter_enable == 2)
                                                {
                                                    obj_box[j].md = 1;
                                                }
                                ...
                                if(filter_enable == 2)
                                {
                                    if(ak_object_filter_alarm(filter_handle, obj_box, output.total_num) != 0)
                                    {
                                        for(int j = 0; j < output.total_num; j++)
                                        {
                                            AK_SVP_RECT_T *src = &output.target_boxes[j];
                                            ak_print_normal(MODULE_ID_APP, 
"SVP Filter Alarm[%d] :Mobile [%s] detect, score[%lu],\
Res is:left[%lu],top[%lu],right[%lu]:bottom[%lu]\n",
                                            j, src->label ==  AK_SVP_HUMAN_SHAPE ?"Body":
                                            "Face",src->score, src->left, src->top, src->right, src->bottom);
                                        }
...
#endif
{
    ...
    || (svp_mode != 0 && svp_mode != 1  && svp_mode != 2 && svp_mode != 3)
...
#ifdef AK_RTOS
SHELL_CMD_EXPORT_ALIAS(svp_sample, ak_svp_sample, svp Sample Program);
#endif
