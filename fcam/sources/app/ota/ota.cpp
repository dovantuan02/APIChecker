...
static void otaRespondProcess(int otaProcess) {
	...
	resJSon["Id"]				  = FCA_GET_SERIAL_INFO;
...
int fca_otaProcess(void *clientp, double dltotal, double dlnow, double ultotal, double ulnow) {
	...
	if ((mtce_otaNextProcess + DISTANCE_RESPOND_PROCESS_OTA) <= process || mtce_otaNextProcess > process || process == 100) {
		if (mtce_otaNextProcess != process) {
			mtce_otaNextProcess = process;
			APP_DBG("\rSend message json process: %d \n", (int)mtce_otaNextProcess);
			otaRespondProcess((int)mtce_otaNextProcess);
		}
	}
...
int fca_otaDownloadFile(const string &url, const string &file, uint32_t timeout) {
	...
	do {
		...
		if (!f) {
			APP_DBG("[ota-download] open file %s fail: %s\n", file.data(), strerror(errno));
			state = FCA_OTA_FAIL;
			break;
		}
		...
		curl_easy_setopt(handle, CURLOPT_VERBOSE, 2L);
		...
		sprintf(path, "%s/%s", FCA_DFAUL_CONF_PATH, FCA_NETWORK_CA_FILE);
		curl_easy_setopt(handle, CURLOPT_CAINFO, path);
